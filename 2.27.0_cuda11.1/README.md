...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmdetection:2.27.0_cuda11.1
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G \
    -v /local/dir:/container/dir \
    -it waikatodatamining/mmdetection:2.27.0_cuda11.1
  ```

### Build local image

* Build the image from Docker file (from within /path_to/mmdetection/2.27.0_cuda11.1)
...
```bash
docker build -t mmdetection:2.27.0_cuda11.1 .
```
...
* Tag
  ...
  ```bash
  docker tag \
    mmdetection:2.27.0_cuda11.1 \
    public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmdetection:2.27.0_cuda11.1
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmdetection:2.27.0_cuda11.1
  ```
...
* Push
  ...
  ```bash
  docker push waikatodatamining/mmdetection:2.27.0_cuda11.1
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
